...
WaypointSaver::WaypointSaver() : private_nh_("~")
{
  ...
  // private_nh_.param<double>("K", K, 0.5);
  // subscriber
...
void WaypointSaver::outputProcessing(geometry_msgs::Pose current_pose, double velocity) const
{
  ...
  else
  {
    ...
    if(stop_once && distance < interval_ && velocity > -0.2 && velocity < 0.1)
    {

     double theta = atan((current_pose.position.y -previous_pose.position.y) /
                        (current_pose.position.x - previous_pose.position.x) );

     if( (current_pose.position.x - previous_pose.position.x)<0.1 &&  (current_pose.position.x - previous_pose.position.x)>-0.1 )
     {
       last_pose.position.x = current_pose.position.x  ;
       last_pose.position.y = current_pose.position.y +  K* (fabs(current_pose.position.y -previous_pose.position.y)/
                                                                   (current_pose.position.y -previous_pose.position.y));
       last_pose.position.z = current_pose.position.z;
       last_pose.orientation = current_pose.orientation;
     ofs << std::fixed << std::setprecision(4) << last_pose.position.x << "," << last_pose.position.y << ","
          << last_pose.position.z << "," << tf::getYaw(last_pose.orientation) << "," << velocity << ",0" << std::endl;
     stop_once = false;       
     }
     else if( (current_pose.position.y - previous_pose.position.y)<0.1 &&  (current_pose.position.y - previous_pose.position.y)>-0.1 )
     {
       last_pose.position.x = current_pose.position.x + K * (fabs(current_pose.position.x -previous_pose.position.x)/
                                                               (current_pose.position.x -previous_pose.position.x)) ;
       last_pose.position.y = current_pose.position.y ;
       last_pose.position.z = current_pose.position.z;
       last_pose.orientation = current_pose.orientation;
     ofs << std::fixed << std::setprecision(4) << last_pose.position.x << "," << last_pose.position.y << ","
          << last_pose.position.z << "," << tf::getYaw(last_pose.orientation) << "," << velocity << ",0" << std::endl;
     ...
     else if(((current_pose.position.x - previous_pose.position.x)>0 && (current_pose.position.y -previous_pose.position.y)>0) or
         ((current_pose.position.x - previous_pose.position.x)>0 && (current_pose.position.y -previous_pose.position.y)<0) ) 
     {
      last_pose.position.x = current_pose.position.x + cos (theta)* K;
      last_pose.position.y = current_pose.position.y + sin (theta)* K;
      last_pose.position.z = current_pose.position.z;
      last_pose.orientation = current_pose.orientation;
     ofs << std::fixed << std::setprecision(4) << last_pose.position.x << "," << last_pose.position.y << ","
          << last_pose.position.z << "," << tf::getYaw(last_pose.orientation) << "," << velocity << ",0" << std::endl;
